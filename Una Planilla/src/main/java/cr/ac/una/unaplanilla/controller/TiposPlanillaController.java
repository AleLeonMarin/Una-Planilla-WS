...
public class TiposPlanillaController extends Controller implements Initializable {
    ...
    private void onActionBtnNuevo(ActionEvent event) {
        if (tbpInclusionEmpleados.isSelected()) {
            ...
            if (new Mensaje().showConfirmation("Limpiar tipo planilla", getStage(),
                    "¿Está seguro que desea limpiar el registro?")) {
                nuevoTipoPlanilla();
            }
        }
    }
    ...
    private void onActionBtnReportes(ActionEvent event) {
        try {
            TipoPlanillaService service = new TipoPlanillaService();
            Respuesta respuesta = service.reporte();

            if (respuesta.getEstado()) {
                new Mensaje().showModal(Alert.AlertType.INFORMATION, "Reporte de tipos de planilla", getStage(),
                        "Reporte generado y abierto correctamente.");
            } else {
                new Mensaje().showModal(Alert.AlertType.ERROR, "Reporte de tipos de planilla", getStage(),
                        respuesta.getMensaje());
            }

        } catch (Exception ex) {
            Logger.getLogger(TiposPlanillaController.class.getName()).log(Level.SEVERE,
                    "Error obteniendo el reporte de tipos de planilla.", ex);
            new Mensaje().showModal(Alert.AlertType.ERROR, "Reporte de tipos de planilla", getStage(),
                    "Ocurrió un error obteniendo el reporte de tipos de planilla.");
        }
    }
